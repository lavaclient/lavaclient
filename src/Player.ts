...
export default class GuildPlayer extends EventEmitter {
  ...
  public constructor(guildId: string, node: LavaSocket, manager: Manager) {
    ...
    if (!guildId) throw new Error("GuildPlayer: Please provide a guild id.");
    this.guildId = guildId;

    if (!node) throw new Error("GuildPlayer: Please provide a socket.");
    this.node = node;

    if (!manager) throw new Error("GuildPlayer: Please provide a manager.");
    ...
    this.on("event", async (event: Lava.Event) => {
      ...
      switch (event.type) {
        ...
        default:
          // @ts-ignore
          emit("error", `Unknown Event Type: ${event.type}`);
      }
  ...
  public pause(pause: boolean = true): Promise<boolean> {
    this.paused = pause;
  ...
  public equalizer(bands: Lava.EqualizerBand[]) {
  ...
  /** Private Stuff */
  _provideServer(server: VoiceServer) {
    this.#server = server;
  }

  _provideState(state: VoiceState) {
    this.#state = state;
  }
